...
- For non-eval items, submission_assets is a keyed record whose values are declaration objects with asset_type and optional required/file constraints. Item auto-eval uses eval_route_id + eval_score_path; the server derives submission_assets/contributor_keys from the route after removing eval_static_inputs, so clients should not send conflicting submission_assets keys. Optional eval_categories_path (default $.categories) stores subcategory scores for display; ranking still uses the main score. Opt into a leaderboard with leaderboard_enabled and leaderboard_order ('desc' or 'asc'); this displays stored scores and does not run a second ranking route.
